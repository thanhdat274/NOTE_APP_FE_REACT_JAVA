...
const ListFolder = () => {
  ...
  const getListFolder = async () => {
    try {
      ...
      setListFolders(data.data.folder);
  ...
  // chi tiết 1 thư mục
  const getByIdFolders = async (id: any) => {
    const dataOne = { id: id, useData: useData };
    setModal1Open(true);
    try {
      const { data } = await foldersDetail(dataOne as any);
      setFolderOne(data?.data?.folder);
    } catch (error: any) {
      toastr.error(error?.response?.data?.message);
    }
  };
  ...
  const removeFolder = async (id: any) => {
    ...
    if (confirm) {
      await removeFolders({ id: id, useData: useData })
        .then(() => {
          toastr.success('Xóa thư mục thành công');
          setListFolders(listFolders.filter((item: any) => item.id !== id));
        })
        .catch((error) => {
          toastr.error(error?.response?.data?.message);
        });
